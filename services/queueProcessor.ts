        ...
        if (needsDirectus) {
          try {
            ...
            const eventData: MaterialTrackingEvent = {
              ...
              event_input_id: [],
              event_output_id: undefined,
            };

            directusEvent = await createEvent(eventData);
            ...
            // Process outgoing materials sequentially
            if (item.outgoingMaterials?.length && directusEvent?.event_id) {
